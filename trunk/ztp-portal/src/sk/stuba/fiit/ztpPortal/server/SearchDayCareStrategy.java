...
public class SearchDayCareStrategy implements SearchStrategy {
	...
	public List<SearchResultList> searchData(String searchString) {
		...
		try {
			...
			String[] fields = new String[] { "name","description","shortDesc" };
		...
		for (int i=0; i<result.size(); i++){
			...
			if (((DayCare)result.get(i)).isActive() && ((DayCare)result.get(i)).isState() ){
			
			SearchResultList item = new SearchResultList();
			item.setModule("Opatrovate¾stvo");
			item.setText(((DayCare)result.get(i)).getShortDesc());
			item.setId(((DayCare)result.get(i)).getId());
			item.setClas(DayCare.class);
			list.add(item);
			}
		}
