...
public class SearchSchoolStrategy implements SearchStrategy {
	...
	public List<SearchResultList> searchData(String searchString) {
		...
		for (int i=0; i<result.size(); i++){
			...
			if (((School)result.get(i)).isActive() && ((School)result.get(i)).isState() ){
			
			SearchResultList item = new SearchResultList();
			item.setModule("PortalEducation");
			item.setText(((School)result.get(i)).getAddress() + ", "+ ((School)result.get(i)).getTown());
			item.setId(((School)result.get(i)).getId());
			item.setClas(School.class);
			list.add(item);
			}
		}
