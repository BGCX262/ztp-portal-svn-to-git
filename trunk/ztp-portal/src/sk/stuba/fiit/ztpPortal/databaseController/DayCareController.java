...
public class DayCareController implements Serializable{
	...
	public static List<DayCare> getNewDayCare(){
		SessionFactory sf = SessionFactoryHolder.getSF();
		Session session = sf.openSession();
		...
		try{
			...
			String SQL_QUERY = "Select dc.id from DayCare dc where dc.state='true' order by dc.id desc";

//		String SQL_QUERY = "Select dc from DayCare dc where dc.state='true' order by dc.id desc limit 3";

			
		Query query = session.createQuery(SQL_QUERY).setMaxResults(3).setCacheable(true).setCacheRegion("RegionName");
	...
	public boolean deleteAllDeacivatedDayCare(){
		SessionFactory sf = SessionFactoryHolder.getSF();
		Session session = sf.openSession();
		long result = -1;
		try{
			session.beginTransaction();
			String SQL_QUERY = "Delete from DayCare dayCare where dayCare.active='false'";
			Query query = session.createQuery(SQL_QUERY);
