 ...
 */
	...
	public static List<Job> getNewJob(){
		SessionFactory sf = SessionFactoryHolder.getSF();
		Session session = sf.openSession();
		...
		try{
			...
			String SQL_QUERY = "Select job.id from Job job where job.state='true' order by job.id desc";
		...
		while (i<list.size()){
			jobId = (Long)list.get(i++);
			jobList.add((Job) session.get(Job.class,jobId));
		}
		}finally{
			session.close();
			sf.close();
	...
	public boolean deleteAllDeactivatedJob(){
		SessionFactory sf = SessionFactoryHolder.getSF();
		Session session = sf.openSession();
		long result = -1;
		try{
			session.beginTransaction();
			String SQL_QUERY = "Delete from Job job where job.active='false'";
			Query query = session.createQuery(SQL_QUERY);
