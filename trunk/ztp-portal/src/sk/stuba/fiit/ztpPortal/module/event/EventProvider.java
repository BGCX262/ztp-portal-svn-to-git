...
public class EventProvider extends SortableDataProvider implements
	...
	public Iterator<Event> iterator(int first, int count) {
		...
		for (final Event event : list) {
			...
			strlen=event.getName().length();
			if(strlen>20) {
				strlen=20;
				event.setName(event.getName().substring(0,strlen)+"...");
			}
			...
			if ((filter.getOwner()!=null) &&(filter.getOwner().getLogin() != null)
					&& !event.getOwner().getLogin().toLowerCase().contains(
			...
			/// preferencia mesta
			if (prefer && !event.getCounty().getName().equals(preferredCounty.getName())) newList.remove(event);
