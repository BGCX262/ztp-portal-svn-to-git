...
function setup_event(elem, eventType, handler) {
	return (elem.attachEvent) ? elem.attachEvent("on" + eventType, handler)
			: ((elem.addEventListener) ? elem.addEventListener(eventType,
					handler, false) : false);
}
...
function keyb_change() {
	var HTMLshow ="<img src=\"../img/klavesnica.bmp\" border=\"0\" alt=\"Virtuálna klávesnica\" />"; 
	var HTMLhide ="<img src=\"../img/hide_klavesnica.bmp\" border=\"0\" alt=\"Virtuálna klávesnica\" />"; 
	document.getElementById("switch").innerHTML = (opened ? HTMLshow
			: HTMLhide);
	opened = !opened;
	if (opened && !vkb) {
		vkb = new VKeyboard("keyboard", // container's id
				keyb_callback, // reference to the callback function
				true, // create the arrow keys or not? (this and the following
						// params are optional)
				...
				"#777", // border color
				"#CCC", // border/font color of "inactive" key (key with no
						// value/disabled)
				"#FFF", // background color of "inactive" key (key with no
						// value/disabled)
				...
				false, // embed VKeyboard into the page? //TOTO DAT TRUE AK sa
						// klavesnica nema prekryvat
...
function getCaretPositions(ctrl) {
	...
	// IE way:
		...
		var start = end = 0;
		try {
			start = Math.abs(document.selection.createRange().moveStart(
					"character", -10000000)); // start

			if (start > 0) {
				try {
					var endReal = Math.abs(ctrl.createTextRange().moveEnd(
							"character", -10000000));
		...
		try {
			end = Math.abs(document.selection.createRange().moveEnd(
					"character", -10000000)); // end
			if (end > 0) {
				try {
					var endReal = Math.abs(ctrl.createTextRange().moveEnd(
							"character", -10000000));
...
function deleteSelection(ctrl) {
	...
	var tmp = (document.selection && !window.opera) ? ctrl.value.replace(/\r/g,
			"") : ctrl.value;
	ctrl.value = tmp.substring(0, insertionS)
			+ tmp.substring(insertionE, tmp.length);
...
function insertAtCaret(ctrl, val) {
	...
	if (gecko && document.createEvent && !window.opera) {
		...
		var tmp = (document.selection && !window.opera) ? ctrl.value.replace(
				/\r/g, "") : ctrl.value;
		ctrl.value = tmp.substring(0, insertionS) + val
				+ tmp.substring(insertionS, tmp.length);
	}
