...
public class PasswordGenerator {
	...
	public static String getPassord(int length) {
	...
	public static String get(Random random, int length, int type) {
		...
		if (type != DIGITS_ONLY && type != LETTERS_ONLY && type != MIXED)
		...
		for (int i = 0; i < length; i++) {
			...
			// characters
			if (LETTERS_ONLY == type || type != DIGITS_ONLY && type_selector) {
				char c = LETTERS.charAt((int) ((double) LETTERS_LENGTH * random
						.nextDouble()));
			...
			// digits
				...
				generated_password.append(NUMBERS
						.charAt((int) ((double) NUMBERS_LENGTH * random
								.nextDouble())));
			}
		}
	...
	public static void main(String[] args) {
		System.out.println(getPassord(5));
	}
}
